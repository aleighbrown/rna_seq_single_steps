...
def load_comparisons():
    comparisons = "config/deseq2_comparisons.yaml"
...
def salmon_files_from_metadata(grp):
    """
    ...
    salmon_dir = get_output_dir(config['project_top_level'], config['majiq_top_level'])

    #build a list with the full path from those sample names
    salmon_files = [os.path.join(MAJIQ_DIR,"builder",x + config['bam_suffix'] + ".majiq") \
                   for x in grp_samples]
    salmon_files = list(set(majiq_files))
    return(salmon_files)
